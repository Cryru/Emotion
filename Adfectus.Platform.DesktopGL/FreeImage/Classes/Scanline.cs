...
namespace FreeImageAPI
{
    ...
    public sealed class Scanline<T> : MemoryArray<T> where T : struct
    {
        ...
        public Scanline(FIBITMAP dib, int scanline)
            : this(dib, scanline, (int) (typeof(T) == typeof(FI1BIT) ? FreeImage.GetBPP(dib) * FreeImage.GetWidth(dib) :
                typeof(T) == typeof(FI4BIT) ? FreeImage.GetBPP(dib) * FreeImage.GetWidth(dib) / 4 :
            ...
            : base(FreeImage.GetScanLine(dib, scanline), length)
        {
            ...
            if (scanline < 0 || scanline >= FreeImage.GetHeight(dib)) throw new ArgumentOutOfRangeException("scanline");
        }
    }
}