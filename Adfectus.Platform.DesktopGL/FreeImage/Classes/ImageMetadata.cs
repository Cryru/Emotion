...
namespace FreeImageAPI.Metadata
{
    ...
    public class ImageMetadata : IEnumerable, IComparable, IComparable<ImageMetadata>
    {
        ...
        /// </summary>
        /// <param name="dib">Handle to a FreeImage bitmap.</param>
        /// <param name="hideEmptyModels">
        /// When <b>true</b>, empty metadata models
        ...
        /// <para>
        /// In case the getter returns <c>null</c> the model is not contained
        /// by the list.
        /// </para>
        ...
        public MetadataModel this[FREE_IMAGE_MDMODEL model]
        {
            get
            {
                for (int i = 0; i < data.Count; i++)
                {
                    ...
                    {
                        if (!data[i].Exists && HideEmptyModels) return null;
        ...
        /// </summary>
        /// <param name="index">
        /// Index of the <see cref="FreeImageAPI.Metadata.MetadataModel" /> within
        /// this instance.
        /// </param>
        /// <returns>
        /// The <see cref="FreeImageAPI.Metadata.MetadataModel" />
        ...
        public List<MetadataModel> List
        {
            get
            {
                if (HideEmptyModels)
                {
                    ...
                    for (int i = 0; i < data.Count; i++)
                    {
                        if (data[i].Exists) result.Add(data[i]);
                    }
        ...
        public bool AddTag(MetadataTag tag)
        {
            for (int i = 0; i < data.Count; i++)
            {
                if (tag.Model == data[i].Model) return data[i].AddTag(tag);
            }
        ...
        public IEnumerator GetEnumerator()
        {
            if (HideEmptyModels)
            {
                ...
                for (int i = 0; i < data.Count; i++)
                {
                    if (data[i].Exists) tempList.Add(data[i]);
                }
        ...
        public int CompareTo(object obj)
        {
            ...
            if (!(obj is ImageMetadata)) throw new ArgumentException("obj");
        ...
        {
            return dib.CompareTo(other.dib);
        }
    }
}