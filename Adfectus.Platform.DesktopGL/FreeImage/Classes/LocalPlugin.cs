...
namespace FreeImageAPI.Plugins
{
    ...
    public abstract class LocalPlugin
    {
        ...
        public LocalPlugin()
        {
            ...
            if ((implementedMethods & MethodFlags.DescriptionProc) != 0) plugin.descriptionProc = DescriptionProc;

            if ((implementedMethods & MethodFlags.ExtensionListProc) != 0) plugin.extensionListProc = ExtensionListProc;

            if ((implementedMethods & MethodFlags.RegExprProc) != 0) plugin.regExprProc = RegExprProc;

            if ((implementedMethods & MethodFlags.OpenProc) != 0) plugin.openProc = OpenProc;

            if ((implementedMethods & MethodFlags.CloseProc) != 0) plugin.closeProc = CloseProc;

            if ((implementedMethods & MethodFlags.PageCountProc) != 0) plugin.pageCountProc = PageCountProc;

            if ((implementedMethods & MethodFlags.PageCapabilityProc) != 0) plugin.pageCapabilityProc = PageCapabilityProc;

            if ((implementedMethods & MethodFlags.LoadProc) != 0) plugin.loadProc = LoadProc;

            if ((implementedMethods & MethodFlags.SaveProc) != 0) plugin.saveProc = SaveProc;

            if ((implementedMethods & MethodFlags.ValidateProc) != 0) plugin.validateProc = ValidateProc;

            if ((implementedMethods & MethodFlags.MimeProc) != 0) plugin.mimeProc = MimeProc;

            if ((implementedMethods & MethodFlags.SupportsExportBPPProc) != 0) plugin.supportsExportBPPProc = SupportsExportBPPProc;

            if ((implementedMethods & MethodFlags.SupportsExportTypeProc) != 0) plugin.supportsExportTypeProc = SupportsExportTypeProc;

            if ((implementedMethods & MethodFlags.SupportsICCProfilesProc) != 0) plugin.supportsICCProfilesProc = SupportsICCProfilesProc;
            ...
            if (registered) PluginRepository.RegisterLocalPlugin(this);
        }
