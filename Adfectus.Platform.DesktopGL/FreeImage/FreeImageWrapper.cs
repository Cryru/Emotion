    ...
    public static partial class FreeImage
    {
        ...
        public static void ValidateAvailability()
        {
            try
            {
                ...
                if (nativeVersion != ExpectedNativeFreeImageVersion) throw new FreeImageException("FreeImage library version mismatch");
            }
        ...
        {
            return Allocate_(width, height, bpp, 0, 0, 0);
        }
        ...
        {
            return AllocateT_(type, width, height, bpp, 0, 0, 0);
        }
        ...
        /// eg: 0xFF0000
        /// </param>
        /// <param name="green_mask">
        /// Green part of the color layout.
        /// eg: 0x00FF00
        /// </param>
        /// <param name="blue_mask">
        ...
        {
            if (palette != null && bpp <= 8 && palette.Length < 1 << bpp) return FIBITMAP.Zero;
            ...
            if (color.HasValue)
            {
                ...
                try
                {
                    ...
                    return AllocateEx_(width, height, bpp, handle.AddrOfPinnedObject(),
            ...
            return AllocateEx_(width, height, bpp, IntPtr.Zero,
                options, palette, red_mask, green_mask, blue_mask);
        }
            ...
            uint red_mask, uint green_mask, uint blue_mask) where T : struct
        {
            ...
            if (color.HasValue)
            {
                if (!CheckColorType(type, color.Value)) return FIBITMAP.Zero;
                ...
                try
                {
                    ...
                    handle = GCHandle.Alloc(buffer, GCHandleType.Pinned);
                    return AllocateExT_(type, width, height, bpp, handle.AddrOfPinnedObject(),
                        options, palette, red_mask, green_mask, blue_mask);
                }
                finally
                {
            ...
            return AllocateExT_(type, width, height, bpp, IntPtr.Zero,
                options, palette, red_mask, green_mask, blue_mask);
        }
        ...
        {
            byte* addr = (byte*) bits;
            if (addr == null || width <= 0 || height <= 0) return FIBITMAP.Zero;

            FIBITMAP dib = AllocateT_(type, width, height, (int) bpp, red_mask, green_mask, blue_mask);
        ...
        /// <paramref name="filename" /> does not exists.
        /// </exception>
        ...
        public static FIBITMAP LoadEx(string filename, FREE_IMAGE_LOAD_FLAGS flags, ref FREE_IMAGE_FORMAT format)
        {
            ...
            if (!File.Exists(filename)) throw new FileNotFoundException(filename + " could not be found.");

            FIBITMAP dib = new FIBITMAP();
            if (format == FREE_IMAGE_FORMAT.FIF_UNKNOWN) format = GetFileType(filename, 0);

            // check if the plugin is capable of loading files
            if (FIFSupportsReading(format)) dib = Load(format, filename, flags);
        ...
        /// <paramref name="dib" /> or <paramref name="filename" /> is null.
        /// </exception>
        ...
        /// filename use <see cref="FREE_IMAGE_FORMAT.FIF_UNKNOWN" />.
        /// </param>
        /// <returns>Returns true on success, false on failure.</returns>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="dib" /> or <paramref name="filename" /> is null.
            ...
            bool unloadSource)
        {
            ...
            if (format == FREE_IMAGE_FORMAT.FIF_UNKNOWN) format = GetFIFFromFilename(filename);
            ...
            if (format != FREE_IMAGE_FORMAT.FIF_UNKNOWN)
                if (FIFSupportsWriting(format) && FIFSupportsExportType(format, GetImageType(dib)))
                {
                    ...
                    finally
                    {
                        ...
                        if (dibToSave != dib) UnloadEx(ref dibToSave);

                        // On success unload the bitmap
                        if (result && unloadSource) UnloadEx(ref dib);
                    }
                }
        ...
        /// </exception>
        /// <exception cref="ArgumentException">
        /// <paramref name="stream" /> is not capable of reading.
        /// </exception>
            ...
            ref FREE_IMAGE_FORMAT format)
        {
            ...
            if (!stream.CanRead) throw new ArgumentException("stream is not capable of reading.");
            ...
            if (!FIFSupportsReading(format)) return FIBITMAP.Zero;
        ...
        /// <returns>Returns true on success, false on failure.</returns>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="dib" /> or <paramref name="stream" /> is null.
        /// </exception>
        /// <exception cref="ArgumentException">
        /// <paramref name="stream" /> cannot write.
        /// </exception>
            ...
            bool unloadSource)
        {
            ...
            if (!stream.CanWrite) throw new ArgumentException("stream is not capable of writing.");

            if (!FIFSupportsWriting(format) || !FIFSupportsExportType(format, GetImageType(dib))) return false;
        ...
        /// <exception cref="FileNotFoundException">
        /// <paramref name="filename" /> does not exists while opening.
        /// </exception>
        ...
        public static FIMULTIBITMAP OpenMultiBitmapEx(
            ...
            bool keep_cache_in_memory)
        {
            if (!File.Exists(filename) && !create_new) throw new FileNotFoundException(filename + " could not be found.");

            if (format == FREE_IMAGE_FORMAT.FIF_UNKNOWN) format = GetFileType(filename, 0);

            FIMULTIBITMAP dib = new FIMULTIBITMAP();
            if (FIFSupportsReading(format)) dib = OpenMultiBitmap(format, filename, create_new, read_only, keep_cache_in_memory, flags);
        ...
        {
            if (stream == null) return FIMULTIBITMAP.Zero;

            if (!stream.CanSeek) stream = new StreamWrapper(stream, true);
            ...
            try
            {
                if (format == FREE_IMAGE_FORMAT.FIF_UNKNOWN) format = GetFileTypeFromHandle(ref io, handle, checked((int) stream.Length));
            ...
            {
                if (!mdib.IsNull) CloseMultiBitmap(mdib, FREE_IMAGE_SAVE_FLAGS.DEFAULT);
        ...
        public static int[] GetLockedPages(FIMULTIBITMAP dib)
        {
            ...
            if (GetLockedPageNumbers(dib, result, ref count))
            {
                ...
                // Fill array
                if (!GetLockedPageNumbers(dib, result, ref count)) result = null;
            }
        ...
        /// </exception>
        /// <exception cref="ArgumentException">
        /// <paramref name="stream" /> can not read.
        /// </exception>
            ...
            out FIMEMORY memory)
        {
            ...
            do
            {
                ...
                WriteMemory(buffer, blockSize, 1, memory);
        ...
        public static void SetResolutionX(FIBITMAP dib, uint res)
        {
            ...
            SetDotsPerMeterX(dib, (uint) (res / 0.0254d + 0.5d));
        }
        ...
        public static void SetResolutionY(FIBITMAP dib, uint res)
        {
            ...
            SetDotsPerMeterY(dib, (uint) (res / 0.0254d + 0.5d));
        }
        ...
        private static bool CompareMetadata(FIBITMAP dib1, FIBITMAP dib2)
        {
            ...
            foreach (FREE_IMAGE_MDMODEL metadataModel in FREE_IMAGE_MDMODELS)
            {
                ...
                if (GetMetadataCount(metadataModel, dib1) == 0) continue;

                FIMETADATA mdHandle = FindFirstMetadata(metadataModel, dib1, out tag1);
                if (mdHandle.IsNull) continue;
        ...
        /// <exception cref="ArgumentNullException">
        /// <paramref name="dib" /> or <paramref name="table" /> is null.
        /// </exception>
                    ...
                    case 1:
                        ...
                        if ((*(byte*) GetScanLine(dib, 0) & 0x80) == 0)
                            for (int y = 0; y < height; y++)
                            {
                                ...
                                {
                                    if ((scanline[x / 8] & mask) > 0) return 2;
        ...
        /// <see cref="FREE_IMAGE_COLOR_DEPTH.FICD_01_BPP_THRESHOLD" />.
        /// </param>
        /// <returns>Handle to a FreeImage bitmap.</returns>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="dib" /> is null.
        ...
        /// with <see cref="FREE_IMAGE_COLOR_DEPTH.FICD_01_BPP_DITHER" />.
        /// </param>
        /// <returns>Handle to a FreeImage bitmap.</returns>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="dib" /> is null.
        ...
        /// <see cref="FREE_IMAGE_COLOR_DEPTH.FICD_01_BPP_DITHER" />.
        /// </param>
        /// <param name="unloadSource">When true the structure will be unloaded on success.</param>
        /// <returns>Handle to a FreeImage bitmap.</returns>
        /// <exception cref="ArgumentNullException">
            ...
            bool unloadSource)
        {
            ...
            if (GetImageType(dib) == FREE_IMAGE_TYPE.FIT_BITMAP)
                switch (conversion & (FREE_IMAGE_COLOR_DEPTH) 0xFF)
                {
                    ...
                    case FREE_IMAGE_COLOR_DEPTH.FICD_08_BPP:
                        ...
                        {
                            bool isGreyscale = IsGreyscaleImage(dib);
                            if (forceGreyscale && !isGreyscale || reorderPalette && isGreyscale) result = ConvertToGreyscale(dib);
                        }
            ...
            if (result.IsNull) return dib;

            if (unloadSource) Unload(dib);
        ...
        /// <param name="minColorDepth">
        /// <b>true</b> to create a bitmap with the smallest possible
        /// color depth for the specified <paramref name="PaletteSize" />.
        /// </param>
        /// <returns>Handle to a FreeImage bitmap.</returns>
        ...
        public static FIBITMAP ColorQuantizeEx(FIBITMAP dib, FREE_IMAGE_QUANTIZE quantize, int PaletteSize, RGBQUAD[] ReservePalette, int bpp)
        {
            unsafe
            {
                ...
                if (bpp == 8)
                {
                    result = ColorQuantizeEx_(dib, quantize, PaletteSize, reservedSize, ReservePalette);
                }
                else if (bpp == 4)
                {
                    temp = ColorQuantizeEx_(dib, quantize, Math.Min(16, PaletteSize), reservedSize, ReservePalette);
                    if (!temp.IsNull)
                    {
                        result = Allocate_((int) GetWidth(temp), (int) GetHeight(temp), 4, 0, 0, 0);
                ...
                {
                    temp = ColorQuantizeEx_(dib, quantize, 2, reservedSize, ReservePalette);
                    if (!temp.IsNull)
                    {
                        result = Allocate_((int) GetWidth(temp), (int) GetHeight(temp), 1, 0, 0, 0);
                        CloneMetadata(result, temp);
        ...
        {
            if (src.IsNull) throw new ArgumentNullException("src");

            if (dst.IsNull) throw new ArgumentNullException("dst");
            ...
            if ((flags & FREE_IMAGE_METADATA_COPY.CLEAR_EXISTING) > 0)
                ...
                {
                    if (!SetMetadata_(model, dst, null, tag)) return -1;
                }
            ...
            foreach (FREE_IMAGE_MDMODEL model in FREE_IMAGE_MDMODELS)
            {
                FIMETADATA mData = FindFirstMetadata_(model, src, out tag);
                if (mData.IsNull) continue;

                do
                {
                    string key = GetTagKey(tag);
                    if (!(keep && GetMetadata_(model, dst, key, out tag2)))
                        if (SetMetadata_(model, dst, key, tag))
                            copied++;
                } while (FindNextMetadata_(mData, out tag));
        ...
        public static string GetImageComment(FIBITMAP dib)
        {
            ...
            FITAG tag;
            if (GetMetadata_(FREE_IMAGE_MDMODEL.FIMD_COMMENTS, dib, "Comment", out tag))
            {
        ...
        public static bool SetImageComment(FIBITMAP dib, string comment)
        {
            ...
            if (comment != null)
            {
                ...
                result = SetMetadata_(FREE_IMAGE_MDMODEL.FIMD_COMMENTS, dib, "Comment", tag);
                DeleteTag(tag);
            }
            else
            {
                result = SetMetadata_(FREE_IMAGE_MDMODEL.FIMD_COMMENTS, dib, "Comment", FITAG.Zero);
            }
            ...
            out MetadataTag tag)
        {
            ...
            FITAG _tag;
            bool result;
            if (GetMetadata_(model, dib, key, out _tag))
            {
            ...
            MetadataTag tag)
        {
            ...
            return SetMetadata_(model, dib, key, tag.tag);
        }
            ...
            out MetadataTag tag)
        {
            ...
            FITAG _tag;
            FIMETADATA result = FindFirstMetadata_(model, dib, out _tag);
            if (result.IsNull)
            {
                tag = null;
                return result;
        ...
        public static bool FindNextMetadata(FIMETADATA mdhandle, out MetadataTag tag)
        {
            FITAG _tag;
            bool result;
            if (FindNextMetadata_(mdhandle, out _tag))
            {
        ...
        {
            if (metaDataSearchHandler.ContainsKey(mdhandle)) metaDataSearchHandler.Remove(mdhandle);
        ...
        {
            return Rotate_(dib, angle, IntPtr.Zero);
        }
        ...
        public static FIBITMAP Rotate<T>(FIBITMAP dib, double angle, T? backgroundColor) where T : struct
        {
            if (backgroundColor.HasValue)
            {
                ...
                try
                {
                    T[] buffer = {backgroundColor.Value};
                    handle = GCHandle.Alloc(buffer, GCHandleType.Pinned);
                    return Rotate_(dib, angle, handle.AddrOfPinnedObject());
                }
                finally
                {
                    if (handle.IsAllocated) handle.Free();
        ...
        public static FIBITMAP Rotate4bit(FIBITMAP dib, double angle)
        {
                ...
                ang % 90 == 0)
            {
                ...
                {
                    case 90:
                        result = Allocate_(height, width, 4, 0, 0, 0);
                        ...
                        break;
                    case 180:
                        result = Allocate_(width, height, 4, 0, 0, 0);
                        if (result.IsNull) break;
        ...
        {
            if (dib.IsNull) return FIBITMAP.Zero;

            if (color.HasValue)
            {
                if (!CheckColorType(GetImageType(dib), color.Value)) return FIBITMAP.Zero;
                ...
                try
                {
                    T[] buffer = {color.Value};
                    handle = GCHandle.Alloc(buffer, GCHandleType.Pinned);
                    return EnlargeCanvas_(dib, left, top, right, bottom, handle.AddrOfPinnedObject(), options);
                }
                finally
                {
                    if (handle.IsAllocated) handle.Free();
            ...
            return EnlargeCanvas_(dib, left, top, right, bottom, IntPtr.Zero, options);
        }
        ...
        ///     <item>
        ///         <term>
        ///             <see cref="FREE_IMAGE_COLOR_OPTIONS.FICO_DEFAULT" />
        ///         </term>
        ///         <description>
        ...
        ///     <item>
        ///         <term>
        ///             <see cref="FREE_IMAGE_COLOR_OPTIONS.FICO_EQUAL_COLOR" />
        ///         </term>
        ///         <description>
        ...
        ///     <item>
        ///         <term>
        ///             <see cref="FREE_IMAGE_COLOR_OPTIONS.FICO_ALPHA_IS_INDEX" />
        ///         </term>
        ///         <description>
            ...
            where T : struct
        {
            ...
            if (!CheckColorType(GetImageType(dib), color)) return false;
        ...
        internal static unsafe string PtrToStr(byte* ptr)
        {
            ...
            if (ptr != null)
            {
                ...
                while (*ptr != 0) sb.Append((char) *ptr++);
        ...
        private static Scanline<FI4BIT>[] Get04BitScanlines(FIBITMAP dib)
        {
            int height = (int) GetHeight(dib);
        ...
        internal static string ColorToString(Color color)
        {
            return string.Format(
                ...
                "{{Name={0}, ARGB=({1}, {2}, {3}, {4})}}", color.Name, color.A, color.R, color.G, color.B);
        }
        ...
        {
            if (array != null && length >= 0 && array.Length != length) Array.Resize(ref array, length);
        }
        ...
        internal static void Resize<T>(ref T[] array, int min, int max)
        {
            if (array != null && min >= 0 && max >= 0 && min <= max)
            {
                ...
                else if (array.Length > max) Array.Resize(ref array, max);
            }
        }
        ...
        internal static bool CheckColorType<T>(FREE_IMAGE_TYPE imageType, T color)
        {
            ...
            {
                case FREE_IMAGE_TYPE.FIT_BITMAP:
                    result = type == typeof(RGBQUAD);
                    break;
                case FREE_IMAGE_TYPE.FIT_COMPLEX:
                    result = type == typeof(FICOMPLEX);
                    break;
                ...
                case FREE_IMAGE_TYPE.FIT_RGB16:
                    result = type == typeof(FIRGB16);
                    break;
                case FREE_IMAGE_TYPE.FIT_RGBA16:
                    result = type == typeof(FIRGBA16);
                    break;
                case FREE_IMAGE_TYPE.FIT_RGBAF:
                    result = type == typeof(FIRGBAF);
                    break;
                case FREE_IMAGE_TYPE.FIT_RGBF:
                    result = type == typeof(FIRGBF);
                    break;
