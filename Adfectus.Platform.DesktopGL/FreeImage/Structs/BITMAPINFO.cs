...
namespace FreeImageAPI
{
    ...
    public struct BITMAPINFO : IEquatable<BITMAPINFO>
    {
        ...
        {
            if (left.bmiHeader != right.bmiHeader) return false;

            if (left.bmiColors == null && right.bmiColors == null) return true;

            if (left.bmiColors == null || right.bmiColors == null) return false;

            if (left.bmiColors.Length != right.bmiColors.Length) return false;

            for (int i = 0; i < left.bmiColors.Length; i++)
            {
                if (left.bmiColors[i] != right.bmiColors[i]) return false;
            }
        ...
        /// <summary>
        /// Tests whether the specified <see cref="BITMAPINFO" /> structure is equivalent to this <see cref="BITMAPINFO" />
        /// structure.
        /// </summary>
        /// <param name="other">A <see cref="BITMAPINFO" /> structure to compare to this instance.</param>
        /// <returns>
        /// <b>true</b> if <paramref name="obj" /> is a <see cref="BITMAPINFO" /> structure
        /// equivalent to this <see cref="BITMAPINFO" /> structure; otherwise, <b>false</b>.
        /// </returns>
