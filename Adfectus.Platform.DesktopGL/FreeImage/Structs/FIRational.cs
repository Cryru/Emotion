...
{
    /// <summary>
    /// The <b>FIRational</b> structure represents a fraction via two <see cref="int" />
    ...
    public struct FIRational : IConvertible, IComparable, IFormattable, IComparable<FIRational>, IEquatable<FIRational>
    {
        ...
        /// </summary>
        public static readonly FIRational MaxValue = new FIRational(int.MaxValue, 1);

        /// <summary>
        /// Represents the smallest possible value of <see cref="FIRational" />. This field is constant.
        /// </summary>
        public static readonly FIRational MinValue = new FIRational(int.MinValue, 1);

        /// <summary>
        /// Represents the smallest positive <see cref="FIRational" /> value greater than zero. This field is constant.
        /// </summary>
        public static readonly FIRational Epsilon = new FIRational(1, int.MaxValue);
        ...
        public FIRational(decimal value)
        {
            try
            {
                ...
                if (Math.Abs(numerator / (decimal) denominator - value) > 0.0001m)
                {
                    int maxDen = int.MaxValue / (int) value - 2;
                    maxDen = maxDen < 10000 ? maxDen : 10000;
                    ApproximateFraction(value, maxDen, out numerator, out denominator);
                    Normalize();
                    if (Math.Abs(numerator / (decimal) denominator - value) > 0.0001m) throw new OverflowException("Unable to convert value into a fraction");
                }
        ...
        {
            return Math.Abs(n * (long) m) / Gcd(n, m);
        }
        ...
        private static int[] CreateContinuedFraction(decimal value)
        {
            ...
            while (value != 0m)
            {
                ...
                value = 1m / value;
                if (Math.Abs(Math.Round(value, precision - 1) - value) < epsilon) value = Math.Round(value, precision - 1);
        ...
        private static void ApproximateFraction(decimal value, int maxDen, out int num, out int den)
        {
            ...
            {
                int numerator = (int) Math.Floor(value * i + 0.5m);
                currentDifference = Math.Abs(value - numerator / (decimal) i);
        ...
        /// <returns>
        /// <b>true</b> if <paramref name="obj" /> is a <see cref="FIRational" /> structure
        /// equivalent to this <see cref="FIRational" /> structure; otherwise, <b>false</b>.
        /// </returns>
        ...
        {
            r2.Normalize();
            if (Math.Abs(r2.numerator) < r2.denominator) return new FIRational(0, 0);
        ...
        {
            return value.denominator == 0 ? 0m : value.numerator / (decimal) value.denominator;
        }
        ...
        {
            return value.denominator == 0 ? 0d : value.numerator / (double) value.denominator;
        }
        ...
        {
            return value.denominator == 0 ? 0f : value.numerator / (float) value.denominator;
        }
        ...
        public int CompareTo(object obj)
        {
            ...
            if (!(obj is FIRational)) throw new ArgumentException();
        ...
        /// <summary>
        /// Tests whether the specified <see cref="FIRational" /> structure is equivalent to this <see cref="FIRational" />
        /// structure.
        /// </summary>
