...
namespace Adfectus.Utility
{
    ...
    public static class MathFloat
    {
        ...
        /// <summary>
        /// Ceiling round a float to a float.
        /// </summary>
        /// <param name="f"></param>
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>
        /// Ceiling round a float to an int.
        /// </summary>
        /// <param name="f"></param>
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        {
            float num = Repeat(b - a, MathExtension.TwoPi);
            if (num > MathExtension.Pi)
                num -= MathExtension.TwoPi;
        ...
        {
            float num = Repeat(target - current, 2 * MathExtension.Pi);
            if (num > MathExtension.Pi)
                num -= 2 * MathExtension.Pi;
        ...
        public static float ApproachAngleRadians(float start, float end, float shift)
        {
            ...
            if (-shift < deltaAngleRadians && deltaAngleRadians < shift)
                return end;
            return Repeat(Approach(start, start + deltaAngleRadians, shift), MathExtension.TwoPi);
        }
        ...
        {
            float time = Engine.TotalTime * speed;
        ...
        {
            angleInDegrees = MathExtension.DegreesToRadians(angleInDegrees);
        ...
        {
            float radians = MathExtension.DegreesToRadians(angleInDegrees);
            return new Vector2
            {
        ...
        {
            float x = Sin(Engine.TotalTime * xFrequency + phase) * xMagnitude;
            float y = Cos(Engine.TotalTime * yFrequency) * yMagnitude;

            return new Vector2(x, y);
        }

        /// <summary>
        /// damped version of a lissajou curve with oscillation between 0 and max magnitude over time. Damping should be between 0
        /// and 1 for best
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Vector2 LissajouDamped(float xFrequency = 2f, float yFrequency = 3f, float xMagnitude = 1, float yMagnitude = 1, float phase = 0.5f, float damping = 0f,
            float oscillationInterval = 5f)
        {
            float wrappedTime = PingPong(Engine.TotalTime, oscillationInterval);
            float damped = Pow(MathExtension.E, -damping * wrappedTime);

            float x = damped * Sin(Engine.TotalTime * xFrequency + phase) * xMagnitude;
            float y = damped * Cos(Engine.TotalTime * yFrequency) * yMagnitude;
