...
namespace Rationale.Utility
{
    public class ObjectDumper
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach (MemberInfo memberInfo in members)
                    {
                        ...
                        {
                            bool enumerable = typeof(IEnumerable).IsAssignableFrom(type);
                            Write("{0}: {1}", memberInfo.Name, enumerable ? "..." : "{ }");

                            bool alreadyTouched = !enumerable && AlreadyTouched(value);
                            _level++;
                            if (!alreadyTouched)
                                DumpElement(value);
                            else
        ...
        private bool AlreadyTouched(object value)
        {
            ...
            foreach (int t in _hashListOfFoundElements)
            {
                if (t == hash)
                    return true;
            }
