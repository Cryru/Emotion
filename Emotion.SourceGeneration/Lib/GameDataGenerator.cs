...
namespace Emotion.GameDataLib
{
    ...
    {
        public static string WriteClassDef(StringBuilder writeTo, Stream gameDataFileStream, out string generatedClassName, out string dataId)
        {
            using (StreamReader reader = new StreamReader(gameDataFileStream))
            {
                return WriteClassDef(writeTo, reader, out generatedClassName, out dataId);
            }
        }

        public static string WriteClassDef(StringBuilder writeTo, string gameDataFileContent, out string generatedClassName, out string dataId)
        {
        ...
        public static string WriteClassDef(StringBuilder writeTo, TextReader reader, out string generatedClassName, out string dataId)
        {
            ...
            string namespac = $"GameData.{dataClassShorthand}Defs";
            generatedClassName = $"{namespac}.{dataId}";

            writeTo.AppendLine("// Generated File");
            writeTo.AppendLine($"namespace {namespac};");
            writeTo.AppendLine();
