...
namespace ImGuiNET
{
    ...
    public unsafe partial struct ImDrawListPtr
    {
        ...
        public ImPtrVector<ImDrawCmdPtr> CmdBuffer
        {
            get => new ImPtrVector<ImDrawCmdPtr>(NativePtr->CmdBuffer, Unsafe.SizeOf<ImDrawCmd>());
        }

        public ImVector<ushort> IdxBuffer
        {
            get => new ImVector<ushort>(NativePtr->IdxBuffer);
        }

        public ImPtrVector<ImDrawVertPtr> VtxBuffer
        {
            get => new ImPtrVector<ImDrawVertPtr>(NativePtr->VtxBuffer, Unsafe.SizeOf<ImDrawVert>());
        }

        public ref ImDrawListFlags Flags
        {
            get => ref Unsafe.AsRef<ImDrawListFlags>(&NativePtr->Flags);
        }

        public ref uint _VtxCurrentIdx
        {
            get => ref Unsafe.AsRef<uint>(&NativePtr->_VtxCurrentIdx);
        }

        public ref IntPtr _Data
        {
            get => ref Unsafe.AsRef<IntPtr>(&NativePtr->_Data);
        }

        public NullTerminatedString _OwnerName
        {
            get => new NullTerminatedString(NativePtr->_OwnerName);
        }

        public ImDrawVertPtr _VtxWritePtr
        {
            get => new ImDrawVertPtr(NativePtr->_VtxWritePtr);
        }
        ...
        {
            get => new ImVector<Vector4>(NativePtr->_ClipRectStack);
        }

        public ImVector<IntPtr> _TextureIdStack
        {
            get => new ImVector<IntPtr>(NativePtr->_TextureIdStack);
        }

        public ImVector<Vector2> _Path
        {
            get => new ImVector<Vector2>(NativePtr->_Path);
        }

        public ref ImDrawCmdHeader _CmdHeader
        {
            get => ref Unsafe.AsRef<ImDrawCmdHeader>(&NativePtr->_CmdHeader);
        }

        public ref ImDrawListSplitter _Splitter
        {
            get => ref Unsafe.AsRef<ImDrawListSplitter>(&NativePtr->_Splitter);
        }

        public ref float _FringeScale
        {
            get => ref Unsafe.AsRef<float>(&NativePtr->_FringeScale);
        }
