    ...
    where TRoomType : ServerGameRoom, new()
{
    ...
    protected override void ProcessMessage(IPEndPoint from, NetworkMessageData msg)
    {
        ...
        switch (messageType)
        {
            case (uint)Network.Base.NetworkMessageType.RequestConnect:
                if (user == null)
                    Msg_RequestConnect(from, msg);
                else
                    SendMessageToIP(from, NetworkMessageHelpers.CreateMessage((uint)Network.Base.NetworkMessageType.Error_AlreadyConnected));
                break;
        }
    ...
    private void Msg_RequestConnect(IPEndPoint ip, NetworkMessageData msg)
    {
        ...
        var connectMessage = NetworkMessageHelpers.CreateMessage((uint)Network.Base.NetworkMessageType.Connected, user.Id);
