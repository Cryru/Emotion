...
public sealed class AndroidAudioLayer : AudioLayer
{
    ...
    {
        AAudio.CreateStreamBuilder(out nint streamBuilder);
        ...
        AAudio.OpenStream(streamBuilder, out nint stream);
    ...
    private void ErrorCallback(nint stream, nint userData, int errorCode)
    {
        Engine.Log.Warning($"Audio layer {Name} encountered error {errorCode}", MessageSource.Audio);
    }

    public unsafe int FillData(nint stream, nint userData, nint dstBufferPtr, int framesGet)
    {
    ...
    protected override unsafe void UpdateBackend()
    {
        ...
        if (Status == PlaybackStatus.Playing)
        {
            ...
            {
                AndroidAudioLayerBuffer buffer = _uploadBuffers[i];
                Assert(buffer.FramesContains >= 0);
                if (!buffer.CanRead)
                {
...
public class AAudio
{
    ...
    public delegate int DataCallback(nint stream, nint userData, nint dstBuffer, int framesGet);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate void ErrorCallback(nint stream, nint userData, int error);
}
