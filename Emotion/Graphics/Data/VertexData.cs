...
public struct VertexData
{
    ...
    )
    {
        ...
        TransformUVs(vertices, texture.Size, (Rectangle)textureArea);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void TransformUVs(Span<VertexData> vertices, Vector2 textureSize, Rectangle uvRect)
    {
        uvRect.Y = textureSize.Y - uvRect.Height - uvRect.Y;
        ...
        float uvYn = textureSize.Y - uvRect.Y;
        float uvYp = textureSize.Y - (uvRect.Y + uvRect.Height);
        Vector2 npUV = new Vector2(uvRect.X, uvYn) / textureSize;
        Vector2 ppUV = new Vector2(uvXP, uvYn) / textureSize;
        Vector2 pnUV = new Vector2(uvXP, uvYp) / textureSize;
        Vector2 nnUV = new Vector2(uvRect.X, uvYp) / textureSize;
    ...
    #region Alternate Vertex Formats

    // temp
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void TransformUVs(Span<VertexData_Pos_UV_Normal_Color> vertices, Vector2 textureSize, Rectangle uvRect)
    {
        uvRect.Y = textureSize.Y - uvRect.Height - uvRect.Y;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void FlipHorizontallySpriteUVs(Span<VertexData_Pos_UV_Normal_Color> vertices)
    {
        // Flipped Y
        // 0, 1    1, 1
        // 0, 0    1, 0
