...
namespace Emotion.Graphics.Objects
{
	...
	public class FrameBuffer : IDisposable
	{
		...
		public FrameBuffer WithDepth(bool bindable = false)
		{
			...
			if (!bindable)
				DepthStencilAttachment = new FrameBufferTexture(this, CreateRenderBuffer(InternalFormat.DepthComponent32F, FramebufferAttachment.DepthAttachment), Size,
		...
		public FrameBufferSampleRequest SampleAsync(Rectangle rect, PixelFormat format, byte[] data = null)
		{
			...
			Engine.CoroutineManager.StartCoroutine(FramebufferSampleRequest_CheckingCoroutine(newFence, (int) byteSize, data));
		...
		private static void EnsureBound(uint pointer)
		{
			...
			if (Bound == pointer && pointer != 0)
			{
				...
				Gl.Get(GetPName.DrawFramebufferBinding, out int actualBound);
