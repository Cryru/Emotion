...
namespace Emotion.Graphics.Shading
{
    public static class ShaderFactory
    {
        ...
        {
            string normalizedNewLinesSource = Helpers.NormalizeNewLines(shaderSource);

            Gl.ShaderSource(shaderId, [normalizedNewLinesSource], [normalizedNewLinesSource.Length]);
            Gl.CompileShader(shaderId);
            ...
            if (lLength > 1) // 0 without null termination
            {
                ...
                Engine.Log.Trace(normalizedNewLinesSource, MessageSource.ShaderSource);
            }
