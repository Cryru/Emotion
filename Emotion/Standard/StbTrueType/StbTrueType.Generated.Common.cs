...
namespace NewStbTrueTypeSharp
{
	unsafe partial class StbTrueType
	{
		...
		public static int stbtt__compute_crossings_x(float x, float y, int nverts, stbtt_vertex* verts)
		{
			...
			float y_frac = 0;
			var winding = 0;
			y_frac = (float)StbRuntime.fmod(y, 1.0f);
			if (y_frac < 0.01f)
				y += 0.01f;
		...
		{
			if (x < 0)
				return -(float)StbRuntime.pow(-x, 1.0f / 3.0f);
			return (float)StbRuntime.pow(x, 1.0f / 3.0f);
		}

		public static void stbtt__h_prefilter(Span<byte> pixels, int w, int h, int stride_in_bytes, uint kernel_width)
		{
			var buffer = stackalloc byte[8];
			var safe_w = (int)(w - kernel_width);
			var j = 0;
			StbRuntime.memclear(buffer, 8);
			for (j = 0; j < h; ++j)
			{
				var i = 0;
				uint total = 0;
				StbRuntime.memclear(buffer, kernel_width);
				total = 0;
				switch (kernel_width)
				{
					case 2:
		...
		{
			var nlen = (int)StbRuntime.strlen((sbyte*)name);
			uint nm = 0;
			uint hd = 0;
			...
			float* hits)
		{
			...
			if (a != 0.0)
			{
				...
				{
					var rcpna = -1 / a;
					var d = (float)StbRuntime.sqrt(discr);
					s0 = (b + d) * rcpna;
					s1 = (b - d) * rcpna;
					if (s0 >= 0.0 && s0 <= 1.0)
						num_s = 1;
		...
		{
			return (top_width + bottom_width) * 0.5f * height;
		}
		...
		public static int stbtt__solve_cubic(float a, float b, float c, float* r)
		{
			...
			{
				var u = (float)StbRuntime.sqrt(-p / 3);
				var v = (float)StbRuntime.acos(-StbRuntime.sqrt(-27 / p3) * q / 2) / 3;
				var m = (float)StbRuntime.cos(v);
				var n = (float)StbRuntime.cos(v - Maths.PI_OVER2) * 1.732050808f;
				r[0] = s + u * 2 * m;
				r[1] = s - u * (m + n);
				r[2] = s - u * (m - n);
				return 3;
			...
			float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n)
		{
			...
			var dx = x3 - x0;
			var dy = y3 - y0;
			var longlen = (float)(StbRuntime.sqrt(dx0 * dx0 + dy0 * dy0) + StbRuntime.sqrt(dx1 * dx1 + dy1 * dy1) +
								   StbRuntime.sqrt(dx2 * dx2 + dy2 * dy2));
			var shortlen = (float)StbRuntime.sqrt(dx * dx + dy * dy);
		...
		public static void stbtt__v_prefilter(Span<byte> pixels, int w, int h, int stride_in_bytes, uint kernel_width)
		{
			var buffer = stackalloc byte[8];
		...
		public static stbtt__point* stbtt_FlattenCurves(
        ...
        )
		{
			...
			*num_contours = n;
			if (n == 0)
				return null;
            *contour_lengths = arena.AllocateOfType<int>(n);
			...
			for (pass = 0; pass < 2; ++pass)
			{
				...
				if (pass == 1)
				{
					points = arena.AllocateOfType<stbtt__point>(num_points);
				}
			...
			float* ypos, stbtt_aligned_quad* q, int opengl_fillrule)
		{
			...
			var round_x = (int)StbRuntime.floor(*xpos + b->xoff + 0.5f);
			var round_y = (int)StbRuntime.floor(*ypos + b->yoff + 0.5f);
			...
			float* ypos, stbtt_aligned_quad* q, int align_to_integer)
		{
			...
			{
				float x = (int)StbRuntime.floor(*xpos + b->xoff + 0.5f);
				float y = (int)StbRuntime.floor(*ypos + b->yoff + 0.5f);
				q->x0 = x;
				q->y0 = y;
