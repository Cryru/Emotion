...
public static class AssimpFormat
{
    ...
    private static unsafe SkeletonAnimRigNode[] WalkNodesForSkeleton(Node* root)
    {
        ...
        queue.Enqueue((nint)root);

        while (queue.TryDequeue(out nint nextNodePtr))
        {
            Node* nodePtr = (Node*)nextNodePtr;

            int parentIdx = -1;
            nint parentPtr = (nint)nodePtr->MParent;
            if (parentPtr != nint.Zero)
            {
    ...
    )
    {
        ...
        for (var p = 0; p < m->MNumFaces; p++)
        {
            ...
            if ((nint)face.MIndices == nint.Zero) continue;
    ...
    {
        if ((nint)meta == nint.Zero) return null;
