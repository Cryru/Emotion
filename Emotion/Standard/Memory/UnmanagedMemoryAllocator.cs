...
public static class UnmanagedMemoryAllocator
{
    ...
    private static ConcurrentDictionary<nint, UnmanagedMemory> _ptrToHandle = new ConcurrentDictionary<nint, UnmanagedMemory>();
    ...
    private static nint AllocInternal(int size, out UnmanagedMemory memoryHandle)
    {
        AllocatedSize += size;
        nint memoryPtr = Marshal.AllocHGlobal(size);
    ...
    /// <returns></returns>
    public static nint MemAlloc(int size)
    {
    ...
    /// <returns></returns>
    public static nint MemAllocNamed(int size, string name)
    {
        nint memoryPtr = AllocInternal(size, out UnmanagedMemory memory);
        memory.Label = name;
    ...
    /// <returns></returns>
    public static nint MemAllocOrReAllocNamed(int size, string name)
    {
        ...
        nint newPtr = Marshal.ReAllocHGlobal(memory.Address, size);
        memory.Size = size;
    ...
    {
        nint newPtr = Marshal.ReAllocHGlobal(ptr, size);
    ...
    public static void RegisterAllocatedMemory(nint ptr, string name, int size)
    {
    ...
    /// <returns></returns>
    public static nint GetNamedMemory(string name, out int size)
    {
        size = 0;
        if (!_labeledMemory.TryGetValue(name, out UnmanagedMemory memoryHandle)) return nint.Zero;
