...
namespace Emotion.Standard.Audio
{
    public static class AudioUtil
    {
        ...
        /// <summary>
        /// Converts float audio data to the specified bits per sample format.
        /// </summary>
        /// <param name="dstFormat">The format to conver to.</param>
        /// <param name="data">The data to convert.</param>
        /// <returns>The converted data.</returns>
        public static unsafe Span<byte> ConvertFloatToBps(AudioFormat dstFormat, Span<float> data)
        {
            // Convert data.
            switch (dstFormat.BitsPerSample)
            {
                ...
                case 32 when !dstFormat.IsFloat: // int
                    fixed (void* dataPtr = &data[0])
                    {
                        var dataInt = new Span<int>(dataPtr, data.Length);
                        for (var i = 0; i < dataInt.Length; i++)
                        {
                            if (data[i] < 0)
            ...
            fixed (void* dataFPtr = &data[0])
            {
                return new Span<byte>(dataFPtr, data.Length * 4);
            }
        }
