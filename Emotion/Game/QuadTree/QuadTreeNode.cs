...
namespace Emotion.Game.QuadTree
{
	...
	public class QuadTreeNode<T> where T : IQuadTreeObject
	{
		...
		private void InsertInternal(QuadTreeObject<T> item)
		{
			...
			Assert(_objects.IndexOf(item) == -1);
			item.Owner = this;
			_objects.Add(item);
		}
		...
		{
			Assert(_runningQueryCount == 0, "Quad tree node is subdividing while a query is running on it. Expect problems.");
		...
		{
			Assert(_runningQueryCount == 0, "Object is moving across nodes while a query is running on its node. Expect problems.");
			...
			if (QuadRect.ContainsInclusive(bounds) || Parent == null)
			{
				...
				dest.Insert(item);
				Assert(formerOwner != null || Parent == null);
			...
			else if (searchArea.Intersects(ref _quadRect) || _quadRect == Rectangle.Empty)
			{
				...
				if (_objects != null)
					for (var i = 0; i < _objects.Count; i++)
					{
						...
						Assert(results.IndexOf(obj) == -1, "QuadTree object found twice in query. Was it added twice?");
		...
		[ThreadStatic] private static QuadTreeQuery<T> _legacyMethodQuery;
