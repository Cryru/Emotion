...
namespace Emotion.Game.Physics2D.Distance
{
    ...
    {
        private static int _maxGJKIterations = 20;
        ...
        public static void ComputeDistance(ref DistanceInput input, out DistanceOutput output, out SimplexCache cache)
        {
            ...
            while (iter < _maxGJKIterations)
            {
                ...
                if (d.LengthSquared() < Maths.EPSILON * Maths.EPSILON)
                ...
                vertex.IndexA = input.ProxyA.GetSupport(input.TransformA.RotateTransposeVector(-d));
                vertex.WA = input.TransformA.TransformVector(input.ProxyA._vertices[vertex.IndexA]);

                vertex.IndexB = input.ProxyB.GetSupport(input.TransformB.RotateTransposeVector(d));
                vertex.WB = input.TransformB.TransformVector(input.ProxyB._vertices[vertex.IndexB]);
            ...
            if (input.UseRadii)
            {
                ...
                if (output.Distance > rA + rB && output.Distance > Maths.EPSILON)
                {
        ...
        public static bool ShapeCast(ref ShapeCastInput input, out ShapeCastOutput output)
        {
            ...
            float radiusA = MathF.Max(proxyA._radius, PolygonShape.PolygonRadius);
            float radiusB = MathF.Max(proxyB._radius, PolygonShape.PolygonRadius);
            float radius = radiusA + radiusB;

            PhysicsTransform xfA = input.TransformA;
            PhysicsTransform xfB = input.TransformB;
            ...
            int indexA = proxyA.GetSupport(xfA.RotateTransposeVector(-r));
            Vector2 wA = xfA.TransformVector(proxyA.GetVertex(indexA));
            int indexB = proxyB.GetSupport(xfB.RotateTransposeVector(r));
            Vector2 wB = xfB.TransformVector(proxyB.GetVertex(indexB));
            Vector2 v = wA - wB;

            // Sigma is the target distance between polygons
            float sigma = MathF.Max(PolygonShape.PolygonRadius, radius - PolygonShape.PolygonRadius);
            float tolerance = 0.5f * PhysicsConfig.LinearSlop;

            // Main iteration loop.
            int iter = 0;

            //Velcro: We have moved the max iterations into settings
            while (iter < _maxGJKIterations && v.Length() - sigma > tolerance)
            {
                ...
                indexA = proxyA.GetSupport(xfA.RotateTransposeVector(-v));
                wA = xfA.TransformVector(proxyA.GetVertex(indexA));
                indexB = proxyB.GetSupport(xfB.RotateTransposeVector(v));
                wB = xfB.TransformVector(proxyB.GetVertex(indexB));
                Vector2 p = wA - wB;
