...
public class ShaderAsset : XMLAsset<ShaderDescription>
{
    ...
    /// </summary>
    public ShaderProgram Shader { get; protected set; } = null!;

    /// <summary>
    /// If true the shader is a single .glsl file rather than an .xml file.
    /// </summary>
    public TextAsset? IsCombinedShader { get; protected set; }
    ...
    protected override void CreateInternal(ReadOnlyMemory<byte> data)
    {
        ...
        {
            IsCombinedShader = new TextAsset();
            IsCombinedShader.Name = Name;
            IsCombinedShader.Create(data);
    ...
    private void Compile(string? compileConstant = null)
    {
        ...
        if (IsCombinedShader != null)
        {
            if (Shader != null) // Reloading, so reread
            {
                TextAsset? textAssetMe = Engine.AssetLoader.Get<TextAsset>(Name, false);
                AssertNotNull(textAssetMe);
                IsCombinedShader = textAssetMe;
            }
            fragShader = IsCombinedShader;
            vertShader = IsCombinedShader;
        }
