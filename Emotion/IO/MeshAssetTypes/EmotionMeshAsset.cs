...
public class EmotionMeshAsset : Asset
{
    ...
    public static MeshEntity? EntityFromByteArray(ReadOnlyMemory<byte> byteArray)
    {
        ...
        if (version > 3)
        {
            Dictionary<string, (Sphere, Cube)> bounds = new Dictionary<string, (Sphere, Cube)>();
            ...
            for (int i = 0; i < boundCount; i++)
            {
                ...
                Sphere sph = new Sphere(pos, radius);

                var cubeOrigin = ReadVector3(reader);
                var cubeExtents = ReadVector3(reader);
                Cube cub = new Cube(cubeOrigin, cubeExtents);

                bounds.Add(name, (sph, cub));
            }
            entity.CachedBounds = bounds;
        }
    ...
    public static byte[] EntityToByteArray(MeshEntity entity)
    {
        ...
        entity.CacheBounds();
        Dictionary<string, (Sphere, Cube)>? bounds = entity.CachedBounds;
        writer.Write(bounds == null ? 0 : bounds.Count);
        if (bounds != null)
        {
            foreach (var boundPair in bounds)
            {
                writer.Write(boundPair.Key);

                var sphere = boundPair.Value.Item1;
                WriteVector3(writer, ref sphere.Origin);
                writer.Write(sphere.Radius);

                var cube = boundPair.Value.Item2;
