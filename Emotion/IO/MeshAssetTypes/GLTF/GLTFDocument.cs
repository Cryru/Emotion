...
public class GLTFDocument
{
    ...
    public ReadOnlyMemory<byte> ReadGltfBuffer(string assetName, GLTFAccessor accessor, out int stride)
    {
        int weightViewIdx = accessor.BufferView;
        var view = BufferViews[weightViewIdx];
        var bufferIdx = view.Buffer;
        var bufferRef = Buffers[bufferIdx];
        string uri = bufferRef.Uri;

        if (view.ByteStride != 0)
            stride = view.ByteStride;
        else
            stride = accessor.GetDataSize();
        ...
        {
            string assetPath = AssetLoader.GetDirectoryName(assetName);
            uri = AssetLoader.GetNonRelativePath(assetPath, uri);
            var byteAsset = Engine.AssetLoader.Get<OtherAsset>(uri, false);
            if (byteAsset == null) return null;
            content = byteAsset.Content;
        }

        return content.Slice(view.ByteOffset, view.ByteLength).Slice(accessor.ByteOffset);
    }
}
