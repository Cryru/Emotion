...
namespace Emotion.Core.Platform;
...
public abstract partial class PlatformBase
{
    ...
    private bool DefaultButtonBehavior(Key key, KeyState state)
    {
        if (Engine.Configuration.DebugMode)
        {
            ...
            switch (key)
            {
                case Key.F11 when state == Core.Systems.Input.KeyState.Down && ctrl:
                    Size = Engine.Configuration.RenderSize * 1.999f - Vector2.One;
                    return false;
                case Key.Pause when state == Core.Systems.Input.KeyState.Down:
                    PerfProfiler.ProfileNextFrame();
                    break;
            }
        }

        if (key == Key.Enter && state == Core.Systems.Input.KeyState.Down && IsAltModifierHeld())
        {
    ...
    protected void UpdateKeyStatus(Key key, bool down)
    {
        ...
        if (wasDown && !down) Engine.Input.ReportKeyInput(key, Core.Systems.Input.KeyState.Up);

        // If it was up, and now is down - it was pressed.
        var downHandled = false;
        if (!wasDown && down) Engine.Input.ReportKeyInput(key, Core.Systems.Input.KeyState.Down);
    ...
    protected void UpdateScroll(float amount)
    {
        if (amount != 0)
        {
            ...
            Engine.Input.ReportKeyInput(Key.MouseWheel, amount < 0 ? Core.Systems.Input.KeyState.Down : Core.Systems.Input.KeyState.Up);
