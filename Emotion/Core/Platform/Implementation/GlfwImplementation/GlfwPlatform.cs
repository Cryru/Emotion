...
public class GlfwPlatform : DesktopPlatform
{
    ...
    protected override void SetupInternal(Configurator config)
    {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && Kernel32.GetModuleHandle("renderdoc.dll") != IntPtr.Zero) Glfw.WindowHint(Glfw.Hint.ContextVersionMinor, 1);
#endif
    ...
    {
        Engine.Input.ReportMouseMove(new Vector2((float) x, (float) y));
    }
