...
namespace Emotion.Core.Platform.Implementation.Win32.Wgl
{
    ...
    public sealed unsafe class WglGraphicsContext : GraphicsContext
    {
        ...
        /// </summary>
        public void Init(nint nativeDeviceHandle, nint windowHandle, Win32Platform platform)
        {
            _platform = platform;
            ...
            Assert(nativeDeviceHandle != nint.Zero);
            ...
            nint rc = createContext(nativeDeviceHandle);
            if (rc == nint.Zero) Win32Platform.CheckError("WGL: Could not create dummy context.", true);
            ...
            if (arbCreateContext)
            {
                ...
                for (var i = 0; i < contextFactory.Count; i++)
                {
                    ...
                    nint handle = CreateContextArb(current);
                ...
                if (_contextHandle == nint.Zero) Win32Platform.CheckError("Creating WGL context");
            }

            if (_contextHandle == nint.Zero)
            {
                _contextHandle = createContext(_dc);
                if (_contextHandle == nint.Zero) Win32Platform.CheckError("Creating WGL legacy context", true);
        ...
        private nint CreateContextArb(GLContextDescription contextDescription)
        {
            ...
            {
                handle = _createContextAttribs(_dc, nint.Zero, attPtr);
            }
        ...
        private int SupportedPixelFormat(nint dc)
        {
            var nativeCount = 0;
            ...
            {
                nativeCount = Gdi32.DescribePixelFormat(dc, 1, (uint) sizeof(PixelFormatDescriptor), nint.Zero);
        ...
        public void MakeSubWindowCurrent(nint windowHandle)
        {
