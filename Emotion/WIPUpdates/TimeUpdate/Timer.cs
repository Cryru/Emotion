...
public static class Interpolation
{
    ...
    public static Vector2 SmoothLerp(Vector2 current, Vector2 target, int speed, float delta)
    {
        return target + (current - target) * MathF.Exp(-speed * delta);
    }

    public static Vector3 SmoothLerp(Vector3 current, Vector3 target, int speed, float delta)
    {
        return target + (current - target) * MathF.Exp(-speed * delta);
    }
}
